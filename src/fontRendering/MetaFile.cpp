...
{
	this->aspectRatio = (double) WINDOW_WIDTH / (double) WINDOW_HEIGHT;
	openFile(file);
	loadPaddingData();
	loadLineSizes();
...
{
	std::map<int, Character*>::iterator it = metaData.find(ascii);
	if (it == metaData.end()) {
		return nullptr;
	}
	return it->second;
...
{
	int result;	
	std::map<std::string, std::string>::iterator it = values.find(variable);
	if (it == values.end()) {
		return 0;
	}
