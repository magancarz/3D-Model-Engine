...
{
	// viewport space
	GLfloat mouseX = static_cast<float>(DisplayManager::get_mouse_x());
	GLfloat mouseY = static_cast<float>(DisplayManager::get_mouse_y());
...
{
	GLfloat x = (2 * mouseX) / DisplayManager::WINDOW_WIDTH - 1;
	GLfloat y = (2 * mouseY) / DisplayManager::WINDOW_HEIGHT - 1;
	return glm::vec2(x, -y);
}
